...
ap.add_argument('-batch_size', type=int, default=20) # 배치 사이즈와 seq_length의 차이?
...
ap.add_argument('-sentvec_dim', type=int, default=25)
...
if WEIGHTS == '':
    ...
    while True: # run constantly to improve the performance
        ...
        if epoch % 2000 == 0: #  save the weights of  model for every 10 epochs
            # model.save_weights('novel_layer2_hidden{}_epoch{}.h5'.format(HIDDEN_DIM, epoch))
            k = 0
            while True:
                output = generate_text(model, GENERATE_SENT_NUM, SENT_SIZE, SENTVEC_DIM, ix_to_char, doc2vec_model) # show output
                file = open('C://users//sunbl//desktop//three_output_{}.txt'.format(k), 'w') # file save
                file.write(output)
                file.close()
                k += 1
